    ...
    with a.body():
        with a.div(id='wrap'):
            with a.div(id='container'):
                with a.div(klass='image-text', id='top-header'):
                    a.img(alt='expanded', src='../../../Images/tableCS.png', style='')
                    with a.h1(style='color:gray; background-color: #ddbbff;border-radius: 6px;width: 1200px;box-shadow: 2px 2px 20px 5px #ddbbff'):
                        a('dbo.Network_Ops_Provider_Network_Prefix_Info')
""" 

                with a.div(klass='flex-container'):
                    with a.div(klass='flex-item'):
                        a.span(klass='mif-table')
                        a('Table Row Count: 120')
                    with a.div(klass='flex-item'):
                        a.i(klass='fas fa-calendar-alt')
                        a('Created On: 2021-06-10 22:03:42')
                    with a.div(klass='flex-item'):
                        a.i(klass='fas fa-calendar-alt')
                        a('Last Modified: 2021-06-10 22:03:42')
                with a.div(klass='flex-container'):
                    with a.div(klass='flex-item'):
                        a.span(klass='mif-table')
                        a('Table Row Count: ' + str(dfTableSpecs.iloc[0, dfTableSpecs.columns.get_loc('PartitionRows')]))
                   
                    with a.div(klass='flex-item'):
                        a.i(klass='fas fa-calendar-alt')
                        a('Created On: ' + str(tableCreatedDate))
