...
dftempParamString = "EXEC [dbo].[usp_WBTableDetails] @DatabaseName = N'NRTCUSTOMDB', @TableName = N'OB_GC_HNS_CompleteDate', @Schema = N'Enrollment'"
...
try:
    ...
    with engine.begin() as conn:
      ...
      result = conn.execute(dftempParamString)

      dftemp = pd.DataFrame(result)

      result = conn.execute("EXEC [dbo].[usp_WBIndexDetails] @DatabaseName = N'NRTCUSTOMDB', @TableName = N'OB_GC_HNS_CompleteDate', @Schema = N'Enrollment'")
      rows = result.all()
      
      queryResultIndexCount = len(rows) 
      
      if queryResultIndexCount > 0:
         dfindexes = pd.DataFrame(result)   
      else:
         dfindexes = pd.DataFrame({'TABLE_NAME': pd.Series(dtype='str'),
         ...
         dfindexes.loc[len(dfindexes)] = list     
         #print('Index Details row count:' + str(count))

      

      result = conn.execute("EXEC [dbo].[usp_WBTableSpecs] @DBName = N'NRTCUSTOMDB', @TableName = N'OB_GC_HNS_CompleteDate', @SchemaName = N'Enrollment'")
      
      rows = result.all()
      queryResultSpecCount = len(rows) 
      
      if queryResultSpecCount > 0:
         dftablespecs = pd.DataFrame(result)   
      else:
...
if queryResultIndexCount > 0: #results of the index retrieval stored proc
   IndexCount = dfindexes['IndexID'].max()
   indexString = ''

   #Create a DataFrame object
   dfIndexDetails_cols = ['IndexName', 'IndexColumns']
   ...
   while i <= IndexCount:
       ...
       newdf = dfindexes.query('IndexID == ' + str(i))
...
dftemp['IdentityColumn'] = dftemp['IdentityColumn'].fillna(0)
dftemp['IdentityColumn'] = dftemp['IdentityColumn'].astype(int)
...
def addChangedRow(i):
    with a.tr(klass='changed'):
        ...
        with a.td():   
            a.div(_t=df.iloc[i, df.columns.get_loc('ORDINAL_POSITION')])
        ...
        with a.td():     
            if df.iloc[i, df.columns.get_loc('IdentityColumn')] == 1:
...
def addNormalRow(i):
    with a.tr():
        ...
        with a.td():     
            a.div(_t=df.iloc[i, df.columns.get_loc('IdentityColumn')])       
...
def addClusteredIndexRow(i):
    with a.tr(klass='clustered'):
        ...
        with a.td():   
            a.div(_t=dfIndexDetails.iloc[i, dfIndexDetails.columns.get_loc('IndexName')])

        with a.td():   
            a.div(_t=dfIndexDetails.iloc[i, dfIndexDetails.columns.get_loc('IndexColumns')])
...
def addNCIndexRow(i):
    with a.tr():
        ...
        #with a.td():
        #       #a.div(_t=df.iloc[i, df.columns.get_loc('IndexName')])
        #       if dfindexes.iloc[i, dfindexes.columns.get_loc('IndexType')] == 1:
        #         titleString = dfindexes.iloc[i, dfindexes.columns.get_loc('IndexName')] + ' clustered'
        #         a.img(alt='Clustered Index', klass='cluster', src='../../../Images/cluster.png', title= titleString)
                ...
                with a.div(klass='panel panel-default panel-collapsible'):
                    ...
                    with a.div(klass='panel-body'):
                        with a.table(border='0', cellspacing='1', klass='dataTable table-hover'):
                            ...
                            i = 0
                            while i < len(dfIndexDetails.index):
                                if dfIndexDetails.iloc[i, dfIndexDetails.columns.get_loc('IndexType')] == 1:
